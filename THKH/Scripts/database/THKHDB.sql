...
BEGIN  
	...
	BEGIN
		INSERT INTO VISIT
		VALUES ('2016-12-29 14:00', 'S9876543E', 'S123', 'Benny Tan', 'Visiting', NULL, 'T1', 1101, 'FIRST_ANS_ID', '', 1)

		INSERT INTO VISIT
		VALUES ('2016-12-30 14:00', 'S9876543E', 'S123', 'Benny Tan', 'Visiting', NULL, 'T1', 1101, 'FIRST_ANS_ID', '', 1)

		INSERT INTO VISIT
		VALUES ('2016-12-30 16:00', 'S9876543E', 'S123', 'Benny Tan', 'Visiting', NULL, 'T1', 1101, 'FIRST_ANS_ID', '', 1)

		INSERT INTO VISIT
		VALUES ('2016-12-31 14:00', 'S9876543E', 'S123', 'Benny Tan', 'Visiting', NULL, 'T1', 1101, 'FIRST_ANS_ID', '', 1)
		
		INSERT INTO VISIT
		VALUES ('2017-01-26 20:00', 'S9876543E', 'S123', 'Benny Tan', 'Visiting', NULL, 'T1', 1101, 'FIRST_ANS_ID', '', 1)  
    END 
END;
...
BEGIN  
	...
	--IF (EXISTS (SELECT nric FROM dbo.PATIENT WHERE patientFullName LIKE '%' + @pPatientFullName + '%' AND bedNo = @pBedNo))  
	BEGIN
		INSERT INTO VISIT(visitRequestTime, patientNric, visitorNric, patientFullName, purpose, reason, visitLocation, bedNo, QaID, remarks,confirm)
		VALUES (@pVisitRequestTime, @pPatientNRIC, @pVisitorNRIC, @pPatientFullName, @pPurpose, @pReason, @pVisitLocation, @pBedNo, @pQaID, @pRemarks, @pConfirm)
	END
END; 
...
BEGIN  
	...
	ELSE
		INSERT INTO VISIT(visitRequestTime, patientNric, visitorNric, patientFullName, purpose, reason, visitLocation, bedNo, QaID, remarks, confirm)
		VALUES (@pVisitRequestTime, @pPatientNRIC, @pVisitorNRIC, @pPatientFullName, @pPurpose, @pReason, @pVisitLocation, @pBedNo, @pQaID, @pRemarks, 1)
END; 
...
BEGIN  
  ...
  BEGIN
    ...
    SET @pVisit_Details = (SELECT (CONVERT(VARCHAR(100), visitRequestTime, 105) + ' ' + CONVERT(VARCHAR(10), visitRequestTime, 108) + ',' +  
                    ...
                    CAST(bedNo AS VARCHAR(100)) + ',' +  CAST(QaID AS VARCHAR(100))  + ',' +  remarks + ',' +  CAST(confirm AS VARCHAR(5)))
    FROM VISIT 
...
BEGIN  
	SET NOCOUNT ON  
	BEGIN TRY
		INSERT INTO MASTER_CONFIG (lowerTempLimit, upperTempLimit, lowerTimeLimit, upperTimeLimit, dateUpdated, updatedBy) 
		VALUES (@pLowTemp, @pHighTemp, @pLowTime, @pHighTime, SWITCHOFFSET(SYSDATETIMEOFFSET(), '+08:00'), @pUpdatedBy)
...
------------------------------------------------------------------------------------------------------------------------------------
GO
CREATE PROCEDURE UPDATE_ACCESS_PROFILE
@pProfileName VARCHAR(100),
@pAccessRights VARCHAR(15),
@pUpdatedBy VARCHAR(100),
@responseMessage INT OUTPUT
