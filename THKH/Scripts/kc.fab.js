        ...
        base.init = function(){
            ...
            if (base.links.length > 0){
                ...
                for (var i = 0; i < base.links.length; i++) {
                    ...
                    var title = base.links[i].target;
                    sub_fab_btns_dom += "<div><button data-link-title='"+base.links[i].target+"' data-link-href='"+(base.links[i].url?base.links[i].url:"")+"' data-link-target='"+((base.links[i].target)?base.links[i].target:"")+"' class='sub_fab_btn' style='"+bg_color_style+"'><span style='"+color_style+"'>"+base.links[i].icon+"</span></button></div>";
            ...
            sub_fab_btns.find('.sub_fab_btn').on('mousedown', function(e){
                if ($(this).attr('data-link-href').length > 0) {
                    if ($(this).attr('data-link-target')){
                        runFunc($(this).attr('data-link-href'));
                    }
                }
...
// Run functions
function runFunc(functionStr) {
    switch (functionStr) {
        case "hideTagsFalse": hideTags(true); break;
        case "checkNric": checkNricWarningDeclaration(); break;
        case "submitAssistReg": checkRequiredFields(); break;
    }
}
